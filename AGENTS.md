...
- T3-D1-A adds a fail-closed Weibo boundary around official `@weibo-ai/weibo-cli@0.8.3`. Production may only run fixed `doctor` and available `statuses` catalog reads; strip token/secret environment variables and never expose raw CLI output. The injected text adapter is contract-only: Weibo setup, login, command activation, and live publish remain closed until the owner completes official OAuth, personal verification, and a zero-cost plan gate.
