...
26. `publish_campaign.execution` defaults to `automatic`. Use `assisted-prepare` only with renderer-produced packages for a Project Profile channel; it creates no platform call and no publication receipt.
27. In an assisted handoff, the owner controls login, challenge/2FA, review, and the final official-UI publish action. Never request browser state, selectors, Cookie data, Profile paths, or platform credentials.
28. Use `assisted-confirm` only after the owner supplies the matching public URL. The runtime must recognize the platform host/path and derive its public post ID; a confirmation receipt is owner-confirmed, not proof of remote creation or collection.
29. Assisted packages with unresolved image, GIF, or video media remain blocked. Do not remove media merely to pass the gate or claim that a requested media type is a validated asset.
30. Owner-assisted receipts participate in 1h/48h/7d plans, but unsupported collectors must remain explicitly `unavailable`; never infer engagement values from a public URL.
