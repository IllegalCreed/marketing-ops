import { randomUUID } from 'node:crypto';
import { chmod, mkdir, readFile, rename, unlink, writeFile } from 'node:fs/promises';
import { join, resolve } from 'node:path';
import { z } from 'zod';
import { MarketingOpsError } from './errors.js';

const handlePattern =
  /^(?=.{3,253}$)(?:[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?\.)+[a-z](?:[a-z0-9-]{0,61}[a-z0-9])?$/;
const didPattern = /^did:[a-z0-9]+:[A-Za-z0-9._:%-]+$/;

const activationSchema = z
  .object({
    schemaVersion: z.literal(1),
    channel: z.literal('bluesky'),
    handle: z.string().regex(handlePattern),
    did: z.string().regex(didPattern),
    enabled: z.literal(true),
    enabledAt: z.iso.datetime({ offset: true }),
  })
  .strict();

export type BlueskyActivation = z.infer<typeof activationSchema>;

function isMissing(error: unknown): boolean {
  return typeof error === 'object' && error !== null && 'code' in error && error.code === 'ENOENT';
}

export class BlueskyActivationStore {
  readonly #directory: string;
  readonly #now: () => string;

  constructor(root: string, now: () => string = () => new Date().toISOString()) {
    this.#directory = resolve(root, 'activations');
    this.#now = now;
  }

  async get(): Promise<BlueskyActivation | null> {
    await this.#ensureDirectory();
    try {
      return activationSchema.parse(JSON.parse(await readFile(this.#path(), 'utf8')) as unknown);
    } catch (error) {
      if (isMissing(error)) return null;
      throw new MarketingOpsError('STORAGE_CORRUPTED', 'Activation storage is corrupted');
    }
  }

  async enable(identity: { handle: string; did: string }): Promise<BlueskyActivation> {
    await this.get();
    const parsedIdentity = z
      .object({
        handle: z.string().regex(handlePattern),
        did: z.string().regex(didPattern),
      })
      .strict()
      .safeParse(identity);
    if (!parsedIdentity.success) {
      throw new MarketingOpsError('INVALID_INPUT', 'Bluesky activation identity is invalid');
    }
    const activation = activationSchema.parse({
      schemaVersion: 1,
      channel: 'bluesky',
      ...parsedIdentity.data,
      enabled: true,
      enabledAt: this.#now(),
    });
    const temporary = join(this.#directory, `.${randomUUID()}.tmp`);
    await writeFile(temporary, `${JSON.stringify(activation, null, 2)}\n`, {
      encoding: 'utf8',
      mode: 0o600,
      flag: 'wx',
    });
    try {
      await rename(temporary, this.#path());
      await chmod(this.#path(), 0o600);
    } finally {
      await unlink(temporary).catch(() => undefined);
    }
    return activation;
  }

  async #ensureDirectory(): Promise<void> {
    await mkdir(this.#directory, { recursive: true, mode: 0o700 });
    await chmod(this.#directory, 0o700);
  }

  #path(): string {
    return join(this.#directory, 'bluesky.json');
  }
}
